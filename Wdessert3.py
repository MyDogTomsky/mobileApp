...
class WestDessert(App):
    def build(self) : 
        ...
        self.pageTurner.current = 'westdessertscreen'
...
class Westdessertsare(BoxLayout)   :
    def __init__(self,app,fourthpage, **kwargs):
        super(Westdessertsare,self).__init__(**kwargs,orientation = 'vertical') 
        ...
        button263.bind(on_release = lambda instance : self.dessertis.goingnextpage('secondscreen'))
    ...
    def bringInfo(self,instance) :
        ...
        if (state == 'down' or state =='normal') : 
            if allergyExclude == [] :
                query030 = (f'SELECT Popularity,Name, Type FROM west_dessert  ORDER BY Popularity ASC')      
                cursor.execute(query030)
                result030 = cursor.fetchall()

            else :         
                query03 = (f'SELECT Popularity,Name, Type FROM west_dessert WHERE ({excludePlural}) ORDER BY Popularity ASC')      
