...
class KorjpMain0(App):
    def build(self) : 
        ...
        self.pageTurner.current = 'korjpm0screen'
...
class KorJpma0are(BoxLayout)   :
    def __init__(self,app,fourthpage, **kwargs):
        super(KorJpma0are,self).__init__(**kwargs,orientation = 'vertical') 
        
        self.fourthpage = fourthpage
        self.main0is = app
        self.pageFourth = FourthPage(app = self.main0is)
        
        button201 = Button(text = '< No Soup MAIN List >',font_size = 22, background_color = '#00ff7f',
        ...
        button26.bind(on_release = lambda instance : self.main0is.goingnextpage('stwoscreen'))
        bottomlayout.add_widget(button26)
        button27 = Button(
             font_size = 18, bold = True, markup = True,text = '[color=#F5F5DC] Home [/color]',halign='center')
        button27.bind(on_release = lambda instance : self.main0is.goingtofirst('firstscreen'))
    ...
    def bringInfo(self,instance) :
        ...
        if state == 'down' : #veg is
            if allergyExclude == [] :
                ...
                query120 = (f'SELECT Popularity,Name, RiceNoodle FROM korjan_main WHERE Soup = 0 AND Veg = 1 AND Spicy = 0 ORDER BY Popularity ASC')      
                cursor.execute(query120)
                result120 = cursor.fetchall()
                #query121 : FOR veg & Yes Spicy & No allergy_dislike
                query121 = (f'SELECT Popularity,Name, RiceNoodle FROM korjan_main WHERE Soup = 0 AND Veg = 1 AND Spicy = 1 ORDER BY Popularity ASC')
                cursor.execute(query121)
                result121 = cursor.fetchall()

            else :   
                #query122 : FOR veg & Non-spicy & exist.allergy_dislike       
                query122 = (f'SELECT Popularity,Name, RiceNoodle FROM korjan_main WHERE Soup = 0 AND Veg = 1 AND Spicy = 0 AND ({excludePlural}) ORDER BY Popularity ASC')      
                cursor.execute(query122,excludeSetting)
                result122 = cursor.fetchall()
                #query123 : FOR veg & Yes spicy & exist.allergy_dislike       
                query123 = (f'SELECT Popularity,Name, RiceNoodle FROM korjan_main WHERE Soup = 0 AND Veg = 1 AND Spicy = 1 AND ({excludePlural}) ORDER BY Popularity ASC')      
            ...
            if allergyExclude == [] :
                ...
                query124 = (f'SELECT Popularity,Name, RiceNoodle FROM korjan_main WHERE  Soup = 0 AND Spicy = 0 ORDER BY Popularity ASC')      
                cursor.execute(query124)
                result124 = cursor.fetchall()
                #query125 : Not veg & Yes Spicy & No allergy_dislike
                query125 = (f'SELECT Popularity,Name, RiceNoodle FROM korjan_main WHERE  Soup = 0 AND Spicy = 1 ORDER BY Popularity ASC')
