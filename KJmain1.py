...
class KorjaMain1(App):
    def build(self) : 
        ...
        self.pageTurner.current = 'korjpm1screen'
...
class KorJpma1are(BoxLayout)   :
    def __init__(self,app,fourthpage, **kwargs):
        super(KorJpma1are,self).__init__(**kwargs,orientation = 'vertical') 
        
        self.fourthpage = fourthpage
        self.main1is = app
        self.pageFourth = FourthPage(app = self.main1is)
        
        button201 = Button(text = '< Yes! Soup MAIN List >',font_size = 22, background_color = '#00ff7f',
        ...
        button26.bind(on_release = lambda instance : self.main1is.goingnextpage('stwoscreen'))
        bottomlayout.add_widget(button26)
        button27 = Button(
             font_size = 18, bold = True, markup = True,text = '[color=#F5F5DC] Home [/color]',halign='center')
        button27.bind(on_release = lambda instance : self.main1is.goingtofirst('firstscreen'))
    ...
    def bringInfo(self,instance) :
        ...
        # Name TEXT PRIMARY KEY NOT NULL, Allergy TEXT, RiceNoodle TEXT
        ...
        if state == 'down' : #veg is
            if allergyExclude == [] :
                ...
                query120 = (f'SELECT Popularity,Name, RiceNoodle FROM korjan_main WHERE Soup = 1 AND Veg = 1 AND Spicy = 0 ORDER BY Popularity ASC')      
                cursor.execute(query120)
                result120 = cursor.fetchall()
                #query121 : FOR veg & Yes Spicy & No allergy_dislike
                query121 = (f'SELECT Popularity,Name, RiceNoodle FROM korjan_main WHERE Soup = 1 AND Veg = 1 AND Spicy = 1 ORDER BY Popularity ASC')
                cursor.execute(query121)
                result121 = cursor.fetchall()

            else :   
                #query122 : FOR veg & Non-spicy & exist.allergy_dislike       
                query122 = (f'SELECT Popularity,Name, RiceNoodle FROM korjan_main WHERE Soup = 1 AND Veg = 1 AND Spicy = 0 AND ({excludePlural}) ORDER BY Popularity ASC')      
                cursor.execute(query122,excludeSetting)
                result122 = cursor.fetchall()
                #query123 : FOR veg & Yes spicy & exist.allergy_dislike       
                query123 = (f'SELECT Popularity,Name, RiceNoodle FROM korjan_main WHERE Soup = 1 AND Veg = 1 AND Spicy = 1 AND ({excludePlural}) ORDER BY Popularity ASC')      
            ...
            if allergyExclude == [] :
                ...
                query124 = (f'SELECT Popularity,Name, RiceNoodle FROM korjan_main WHERE  Soup = 1 AND Spicy = 0 ORDER BY Popularity ASC')      
                cursor.execute(query124)
                result124 = cursor.fetchall()
                #query125 : Not veg & Yes Spicy & No allergy_dislike
                query125 = (f'SELECT Popularity,Name, RiceNoodle FROM korjan_main WHERE  Soup = 1 AND Spicy = 1 ORDER BY Popularity ASC')
                cursor.execute(query125)
                result125 = cursor.fetchall()

            else :   
                #query126 : Not veg & Non-spicy & exist.allergy_dislike       
                query126 = (f'SELECT Popularity,Name, RiceNoodle FROM korjan_main WHERE Spicy = 0 AND ({excludePlural}) ORDER BY Popularity ASC')      
                cursor.execute(query126,excludeSetting)
                result126 = cursor.fetchall()
                #query127 : Not veg & Yes spicy & exist.allergy_dislike       
                query127 = (f'SELECT Popularity,Name, RiceNoodle FROM korjan_main WHERE Spicy = 1 AND ({excludePlural}) ORDER BY Popularity ASC')      
        ...
        if result120 :
            ...
            for i in result120 :
                outcome +="[color=#F5F5DC]{0})[/color] [color=#33ab64]Name[/color][color=#F5F5DC] : {1},[/color] [color=#3983e3]Type[/color][color=#F5F5DC] : {2} \n[/color]" .format(i[0],i[1],i[2])             
        if result121 :
